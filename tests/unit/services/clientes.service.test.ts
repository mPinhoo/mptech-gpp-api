...
describe('ClientesService', () => {
  ...
  describe('findById', () => {
    it('deve retornar cliente com pedidos', async () => {
      ...
      mockPrisma.cliente.findFirst.mockResolvedValue(cliente);
    ...
    it('deve lançar NotFoundError se cliente inativo', async () => {
      mockPrisma.cliente.findFirst.mockResolvedValue({ id: '1', ativo: false, pedidos: [] });

      await expect(service.findById(USER_ID, '1')).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('create', () => {
    it('deve criar cliente', async () => {
      ...
      const result = await service.create(USER_ID, { nome: 'Novo', email: 'n@t.com' });
  ...
  describe('delete (soft)', () => {
    it('deve desativar cliente', async () => {
      mockPrisma.cliente.findFirst.mockResolvedValue({ id: '1', ativo: true });
